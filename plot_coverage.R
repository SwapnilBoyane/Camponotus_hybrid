...
rm(a_rep)
save.image("camphybrid_coverage.RData")



load("camphybrid_coverage.RData")

par(mfrow=c(4,6))
for(a in 1:length(output2)) {
	plot(0:51, output2[[a]], pch=19, cex=0.1, xlab="Coverage", ylab="Proportion camphybrid genome", main=sample_names[a], ylim=c(0,0.14))
